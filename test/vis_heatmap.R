#' Heatmap
#'
#' Generates a heatmap of data by using sample metadata to aggregate samples and variable metadata to aggregate variables.
#'
#' @usage vis_heatmap(data, group_by = "")
#'
...
#' @param var_aggregate A variable in the variable metadata to aggregate counts into.
#' @param var_add Additional variable metadata to display, e.g. taxonomy (\code{ampvis2}) or variable in genedata (\code{mmt}).
#' @param var_show The number of variables to show, or a vector of variable names. (\emph{default:} \code{10})
...
amp_heatmap <- function(data,
  ...
  if (!isTRUE(textmap)) {
    ...
    if(isTRUE(plot_functions)) {
      ...
      amp_heatmap_function <- list(heatmap = heatmap,
